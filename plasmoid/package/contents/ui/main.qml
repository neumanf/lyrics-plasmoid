...
Item {
    ...
    Plasmoid.fullRepresentation: Item {
        ...
        function parseInput() {
            var artistAndSong = input.text.split("-");
            ...
            if(artistAndSong.length != 2 || artistAndSong[0].trim().length == 0 || artistAndSong[1].trim().length == 0){
                ...
                var artist = artistAndSong[0].trim();
                var song = artistAndSong[1].trim();
        ...
        function handleSearch() {
            ...
            if(input.error){
                ...
                apiWorker.sendMessage({ 'artist': input.artist, 'song': input.song });
            }
        }

        WorkerScript {
            id: apiWorker
            source: "../modules/lyricsovh.mjs"
            onMessage: {
                textArea.remove(0, textArea.length);
                textArea.insert(0, messageObject.lyrics);
                textArea.select(0, 1);
                textArea.deselect();
            }
        }
